...
describe("computeDoctorIssues", () => {
	it("returns no issues when everything is healthy", () => {
		const input = makeInput({
			listings: [
				{ id: "kroki", status: "registered", tags: ["mermaid"], aliases: {} },
			],
		});
	...
	it("reports unavailable processor with install hint", () => {
		...
		const issues = computeDoctorIssues(input);
		expect(issues).toHaveLength(1);
		expect(issues[0].message).toContain("graphviz-local is unavailable");
	...
	it("reports disabled processor when tags lose their only processor", () => {
		const input = makeInput({
			listings: [
				{ id: "kroki", status: "disabled", tags: ["mermaid", "graphviz"], aliases: {} },
			],
		});

		const issues = computeDoctorIssues(input);
		expect(issues).toHaveLength(1);
		expect(issues[0].message).toContain("kroki is disabled");
	...
	it("does not report disabled processor when another processor covers the tags", () => {
		const input = makeInput({
			listings: [
				{ id: "graphviz-local", status: "registered", tags: ["graphviz"], aliases: {} },
				{ id: "kroki", status: "disabled", tags: ["graphviz", "mermaid"], aliases: {} },
			],
		});

		const issues = computeDoctorIssues(input);
		// graphviz is covered by graphviz-local; only mermaid is orphaned.
		expect(issues).toHaveLength(1);
