/**
 * Vendored from pi-mono @ upstream/main aa1b587b (file last touched at
 * 41377ee8 — "fix(tui): sync test awaits with throttled render pipeline").
 ...
 *
 * Why vendored and not imported: pi-tui does not export VirtualTerminal
 * from its published entry point (dist/index.d.ts). When it does, switch
 * to an import and delete this file.
 *
 * Changes from upstream:
 *   - Imports `Terminal` from `@mariozechner/pi-tui` (the published
 *     surface) rather than from a relative `../src/terminal.js` path.
 *   - Appends `LoggingVirtualTerminal`, shaped verbatim after the
 *     test-local subclass in pi-tui's tui-render.test.ts. Used by
 *     pi-fence's render-layer tests to assert on the exact byte stream
 *     pi-tui emits (e.g. Kitty graphics `\x1b_G...\x1b\\` sequences).
 */
...
/**
 * VirtualTerminal that also records every write as a captured string.
 * Shape lifted verbatim from pi-tui's own tui-render.test.ts pattern —
 * staying in-idiom with upstream.
 *
 * Useful for asserting on the exact byte stream pi-tui emits (escape
 * sequence families like Kitty graphics `\x1b_G...\x1b\\` that the
 * viewport grid alone can't see).
 */
