...
- `kroki.endpoint` now fails closed unless it is a credential-free `http:` or `https:` URL; endpoint paths are preserved, trailing slashes are normalized, and query strings, hash fragments, credentials, unsupported schemes, malformed strings, bare query/hash delimiters, credential delimiters, and whitespace/control-wrapped values are rejected.
