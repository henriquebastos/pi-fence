...
1. **Spec** — draft the story file as a reusable contract, not an implementation transcript. Every story file starts with a visible metadata block carrying `**Status:** Draft|Ready|Done`; active implementation is represented by beans, not `In progress` story status. Every story file must also contain, at minimum: `Summary`, `Done criterion`, `Scope`, `Plan`, `Tests`, and `Verification`. The `Plan` section is acceptance-oriented: stable behavior groups tied to user value, not TDD micro-steps or execution order. The `Tests` section names layers touched, events covered, fakes added, live tests, and anything deferred. Amend spec churn *into the spec commit* so plan revisions don't leak into history. When starting a new story, read the closest finished story file as the working template — imitate the section shape, don't invent one. Parent docs link downward; they do not copy story-level detail.
2. **Implement** — use beans as the mutable execution plan and ledger. One bean at a time, test-first (red → green → refactor), including inspection findings. Use `pnpm test:watch` for the red/green loop when helpful, then `pnpm run feedback` for the fast refactor loop. When the TDD session feels done, run `pnpm run inspect`; findings become beans and re-enter the same TDD micro-loop. After a story is `Ready`, do not edit its file for TDD micro-steps, inspection findings, implementation order, or discovered cleanup tasks. A story is not considered done until its implementation exists in committed history. Do not start the next story with uncommitted carry-over from the previous one.
3. **Close** — close only from a clean working tree after the story's implementation commits already exist. When this story closes an epic, run the acceptance gate first (`pnpm test:live` + `pnpm run render:verify`). Flip the story file metadata to `**Status:** Done`, update status in the epic file, the CV `README.md`, and the top-level roadmap `README.md`, append a worklog entry (commits + test-count deltas + design decisions + known deviations + carry-forwards), and update `CHANGELOG.md`, `README.md`, and `docs/getting-started.md` if user-visible behavior changed. Actual command output, commit lists, test-count deltas, deviations, and carry-forwards live in bean close reasons and the worklog, not in the story file.
