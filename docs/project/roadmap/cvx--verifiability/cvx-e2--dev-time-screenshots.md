...
**Last updated:** 2026-04-20 — S4 Done; the CVx.E2 epic's Planned row is empty
...
| Code | Story | Status |
|------|-------|--------|
| [S1](cvx-e2-s1--headless-image-verifier.md) | **`pnpm render:verify` produces a diffable PNG of one named pi-fence scenario, headlessly; a pixel-diff test gates regressions** | ✅ Done |
| [S2](cvx-e2-s2--multi-scenario-gallery.md) | **Multi-scenario rendering with a per-run HTML gallery; variants plumbing ready for future theme/width matrices** | ✅ Done |
| [S3](cvx-e2-s3--sentinel-readiness.md) | **Sentinel-based readiness; edit-verify loops complete in under five seconds per scenario** | ✅ Done |
| [S4](cvx-e2-s4--user-agent-trail.md) | **`mermaid-user-agent-trail` scenario — full user→assistant→pi-fence:output visual via pi-coding-agent's own components** | ✅ Done |
