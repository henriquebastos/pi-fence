...
**Docs are checked too.** Link integrity is verified by `pnpm run lint:markdown:links`, which walks the `docs/` tree and validates that every relative markdown link resolves to a real file and every `#fragment` points to a real heading. Structural markdown checks — list numbering, blank-line rules, heading increments, duplicate headings — are handled by `markdownlint-cli2` via `pnpm run lint:markdown:body`. Auto-fix most issues with `pnpm run lint:markdown:fix`. `pnpm run lint:markdown` is the umbrella that runs both, and `pnpm run lint` is the convenience alias for that docs-only lane.

**Static typing and architectural dependency rules are part of the fast gate.** `pnpm run lint:types` runs `tsc --noEmit` across production code, tests, and repo scripts. `pnpm run lint:deps` runs dependency-cruiser against the repo's architectural import boundaries, starting with the rule that production code under `extensions/**` must not import from `tests/**`. `pnpm test` also enforces the current extension-coverage minimums: statements `90`, lines `90`, functions `90`, branches `75`. `pnpm run feedback` is the contributor-facing umbrella for the full local fast gate: `pnpm test`, `pnpm run inspect:crap:ext`, `pnpm run lint:markdown`, `pnpm run lint:types`, and `pnpm run lint:deps`.
...
**Refactoring in cycle.** After implement + test, evaluate refactoring. Use the fast loop (`pnpm run feedback`) while simplifying. Run `pnpm run inspect` when the TDD session feels done. Run `pnpm test:live` when adding or changing a processor. Before closing an epic, run the acceptance gate (`pnpm test:live` + `pnpm run render:verify`) — no acceptance criterion depends on human review. Quality targets: focused extension CRAP at or below `25`, Sonar pushed toward `0` open issues when configured. Document what was refactored *and* what was evaluated but left as-is, with criteria for revisiting.
