...
**Refactoring in cycle.** After implement + test, evaluate refactoring. Use the fast loop (`pnpm run feedback`) while simplifying, then run the broader completion inspection pass (`pnpm run inspect`, plus `pnpm test:live` / `pnpm run render:verify` when relevant) before calling the work done. The current completion-pass targets are: focused extension CRAP at or below `25`, and Sonar pushed toward `0` open issues when the experiment is configured. Document what was refactored *and* what was evaluated but left as-is, with criteria for revisiting.
