...
- `bundle-sandbox` — uses a labelled `pi-fence-bundle` Docker exec container or an opt-in Gondolin VM runtime to render `graphviz`/`dot` and `mermaid` without host binaries or remote HTTP when `sandbox` placement is selected and the bundle runtime is ready. Gondolin auto-start requires an explicit image from non-project config.
